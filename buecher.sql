...
CREATE ALGORITHM=UNDEFINED DEFINER=CURRENT_USER() SQL SECURITY DEFINER VIEW `biblio`.`BTausch` AS select concat(cast(`klasse_berechnen`(`biblio`.`schueler`.`vajahr`) as char charset utf8),ifnull(`biblio`.`schueler`.`kbuchst`,'')) AS `Klasse`,`biblio`.`schueler`.`name` AS `Name`, ifnull(concat(`biblio`.`buch`.`name`,' ', `biblio`.`buch`.`jgst`), `biblio`.`buch`.`name`) AS `Buch`,`biblio`.`btausch`.`datum` AS `Datum` from ((`biblio`.`btausch` left join `biblio`.`schueler` on((`biblio`.`btausch`.`sid` = `biblio`.`schueler`.`id`))) left join `biblio`.`buch` on((`biblio`.`btausch`.`bid` = `biblio`.`buch`.`isbn`)));
